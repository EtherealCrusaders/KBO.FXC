    ...
    public class KBOEffectCompilerTask : Microsoft.Build.Utilities.Task
    {
        ...
        public override bool Execute()
        {
            ...
            foreach (var item in EffectFiles ?? Array.Empty<ITaskItem>())
            {
                ...
                //HResult result = D3DCompiler.D3DCompile(new ArraySegment<byte>(contents), fullFxFilePath, new D3DShaderMacros(new Dictionary<string, string?>()
                //{
                //    ["KBOFXC"] = "1"
                //}), new ID3DIncludeNewHandler(fullFxFilePath, AdditionalSearchPaths), entryPoint: null, target: "fx_2_0", compilerFlags, 0, out byte[] effectCode, out string? errors);
